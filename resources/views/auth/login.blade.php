<x-guest-layout>
    <!-- Session Status -->
    @if(session('status'))
        <div style="margin-bottom: 1.5rem; background: var(--success-light); color: var(--success); padding: 0.875rem; border-radius: 12px; font-size: 0.875rem; text-align: center; font-weight: 600; border: 1px solid rgba(0,210,106,0.1);">
            {{ session('status') }}
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 2rem;">
            <label for="remember_me" style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem; color: #64748b; cursor: pointer; font-weight: 500;">
                <input id="remember_me" type="checkbox" name="remember" style="width: 1.125rem; height: 1.125rem; border-radius: 6px; accent-color: var(--primary); cursor: pointer;">
                <span>Ingat Saya</span>
            </label>

            @if (Route::has('password.request'))
                <a style="font-size: 0.875rem; color: var(--primary); font-weight: 700; text-decoration: none;" href="{{ route('password.request') }}">
                    Lupa Password?
                </a>
            @endif
        </div>
        ...
        <div style="margin-top: 2rem; text-align: center; font-size: 0.9375rem; color: #64748b;">
            Belum punya akun? 
            <a href="{{ route('register') }}" style="color: var(--primary); font-weight: 700; text-decoration: none;">Daftar Sekarang</a>
        </div>
        @endif
    </form>
</x-guest-layout>
