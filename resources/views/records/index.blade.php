<x-app-layout>
    ...
    <div class="card"
        style="padding: 0; overflow: hidden; margin-bottom: 2rem; border: 1px solid var(--border-light); animation: fadeInUp 0.4s ease-out;">
        <div
                ...
                <thead>
                    <tr>
                        ...
                        <th
                            ...
                            Guru Pencatat</th>
                        <th
                            style="padding: 1.25rem 1.5rem; background: #f8fafc; font-size: 0.75rem; font-weight: 800; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid var(--border-light); width: 100px;">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($violation_records as $r)
                        <tr class="premium-row">
                            <td
                                style="padding: 1.25rem 1.5rem; font-size: 0.8125rem; font-weight: 700; color: var(--text-muted);">
                                {{ \Carbon\Carbon::parse($r->date)->translatedFormat('d M Y') }}</td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="font-weight: 800; color: var(--primary-dark); font-size: 0.9375rem;">
                                    {{ $r->student->name ?? '-' }}</div>
                                <div style="font-size: 0.6875rem; color: var(--text-muted); font-weight: 600;">Kelas
                                    {{ $r->student->schoolClass->name ?? '-' }}</div>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="font-weight: 800; font-size: 0.875rem; color: #1e293b;">
                                    {{ $r->violationType->name ?? '-' }}</div>
                                <div
                                    style="font-size: 0.6875rem; color: #ef4444; font-weight: 700; text-transform: uppercase; letter-spacing: 0.02em;">
                                    {{ $r->violationType->category ?? '' }}</div>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <span
                                    style="font-weight: 900; color: #ef4444; font-size: 1.125rem;">-{{ $r->violationType->points ?? 0 }}</span>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                    <div
                                        style="width: 32px; height: 32px; border-radius: 10px; background-image: url('https://ui-avatars.com/api/?name={{ urlencode($r->user->name ?? 'U') }}&background=2B5EA7&color=fff&bold=true&size=64'); background-size: cover; border: 2px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                                    </div>
                                    <span
                                        style="font-size: 0.8125rem; font-weight: 700; color: var(--primary-dark);">{{ $r->user->name ?? '-' }}</span>
                                </div>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="display: flex; gap: 0.5rem;">
                                    ...
                                    <form method="POST" action="{{ route('records.destroy', $r) }}"
                                        ...
                                        <button type="submit" class="btn"
                                            style="background: #fee2e2; color: #ef4444; width: 32px; height: 32px; border-radius: 10px; padding: 0; display: flex; align-items: center; justify-content: center; transition: all 0.2s;"
    ...
    <div class="card"
        style="padding: 0; overflow: hidden; border: 1px solid var(--border-light); animation: fadeInUp 0.5s ease-out;">
        <div
                ...
                <tbody>
                    @forelse($vitamin_records as $r)
                        <tr class="premium-row">
                            ...
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="font-weight: 800; font-size: 0.875rem; color: #1e293b;">
                                    {{ $r->vitaminType->name ?? '-' }}</div>
                                <div
                                    style="font-size: 0.6875rem; color: #10b981; font-weight: 700; text-transform: uppercase; letter-spacing: 0.02em;">
                                    {{ $r->vitaminType->category ?? '' }}</div>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <span
                                    style="font-weight: 900; color: #10b981; font-size: 1.125rem;">+{{ $r->vitaminType->points ?? 0 }}</span>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                    <div
                                        style="width: 32px; height: 32px; border-radius: 10px; background-image: url('https://ui-avatars.com/api/?name={{ urlencode($r->user->name ?? 'U') }}&background=10b981&color=fff&bold=true&size=64'); background-size: cover; border: 2px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                                    </div>
                                    <span
                                        style="font-size: 0.8125rem; font-weight: 700; color: var(--primary-dark);">{{ $r->user->name ?? '-' }}</span>
                                </div>
